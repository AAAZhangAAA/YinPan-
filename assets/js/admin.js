...
(function () {
  ...
  function initAdmin() {
    ...
    /* ---------- 直接保存到 GitHub ---------- */
    var ghRepo = $("#ghRepo"), ghToken = $("#ghToken"), ghStatus = $("#ghStatus");
    try {
    ...
    function ghSetStatus(msg, ok) {
      if (ghStatus) {
        ghStatus.textContent = msg;
        ghStatus.style.color = ok ? "#10b981" : "#ef4444";
      }
    }
    ...
    $("#ghPush").addEventListener("click", function () {
      ...
      fetch(api, { headers: headers })
        ...
        .then(function () {
          ghSetStatus("✅ 已保存到 GitHub！GitHub Pages 约 1–2 分钟后自动更新，粉丝即可看到。", true);
          added = []; saveAdded(); renderAdded();
          toast("已保存到 GitHub");
        })
        .catch(function (err) {
          ghSetStatus("❌ " + err.message + "\n（令牌需有 repo 权限；若 401 请重新生成 PAT）", false);
        });
    });
